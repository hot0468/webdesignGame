...
| 날짜 | 변경 | 대상 |
|------|------|------|
| 2026-08-17 | **답장이 다음 날 온다** — 회신하면 곧바로 떠서 새 글인 줄도 모르고 공정의 줄도 안 보이던 것을 고쳤다. 미루는 자리는 `Message.day` 한 칸이고 `inbox()`가 주차+요일을 함께 본다 | src/data/inbox.ts(+test), src/systems/clock.ts(tomorrow,+test)·pipeline.ts·followup.ts, src/store.ts, src/components/Desktop·MessageList.tsx, src/programs/Mail·Talk·Company.tsx |
| 2026-08-17 | **하루가 짧다는 걸 화면이 말한다** — 12:40에 한 시간짜리를 눌렀더니 다음 날이 되던 오해를 풀었다(레벨1은 13:00 퇴근). 계기판이 퇴근 시각을, 버튼이 `내일 이어서`를 적는다 | src/store.ts(useClock.spill), src/components/Hud.tsx·JobActions.tsx, src/programs/Figma·Photoshop·Ppt·Editor.tsx |
| 2026-08-17 | **한 주가 이레가 됐다** — 닷새만 돌아 달력의 토·일이 죽은 칸이던 것을 풀었다. 주말은 **정신력을 무는 날**로 남아(그날 첫 일에 한 번) 쉬면 시간을, 일하면 정신력을 잃는 저울질이 된다. 계기판에 날짜가 선다 | src/data/game.ts(WORKDAY_COUNT·isWeekendDay), src/store.ts(+test), src/systems/calendar.ts·clock.test.ts, src/components/Hud.tsx, src/programs/Schedule.tsx |
