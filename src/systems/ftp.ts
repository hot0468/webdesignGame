 ...
 *
 * 접속 정보의 정본은 그 업체의 `ftp`다(사내시스템 > 업체정보가 보여 주는 그 값). 여기에
 * 값을 다시 적지 마라 — 사내시스템이 보여 주는 것과 에디터가 받아 주는 것이 갈리는 순간
 * "정보를 찾아 옮겨 적는다"는 이 게임의 동선이 끊긴다(`url.ts`의 `checkLogin`과 같은 이유).
 *
 * ⚠️ **업체 목록을 인자로 받는다.** 수주로 생겨난 업체는 상수 `CLIENTS`에 없으므로
 *    여기서 그것을 직접 보면 낙찰로 딴 사이트를 영영 연결할 수 없다 — 목록의 정본은
 *    `clientsOf(jobs)` 하나이고, 부르는 쪽이 그것을 넘긴다.
 *
...
export function checkFtp(input: FtpInput, clients: readonly Client[]): string | undefined {
