 ...
 * 공고는 ①참가 조건을 맞춰야 응모할 수 있고 ②응모해도 **추첨**이라 떨어질 수 있다.
 * 그래서 이쪽의 값은 셋으로 갈린다: **참가 조건**(못 맞추면 응모 불가) · **응모 비용**
 * (행동력) · **당첨 확률 계수**(평판·능력치가 민다).
 *
...
/** 한 주에 뜨는 공고 수. ⚠️ 응모 비용(`BID_AP`)과 함께 읽어야 한다 —
 *  공고가 행동력보다 훨씬 많으면 "무엇에 걸까"가 선택이 되고, 적으면 전부 걸게 된다. */
export const LISTINGS_PER_WEEK = 4

/** 응모 한 건의 값(행동력). ⚠️ **0으로 두지 마라** — 공짜면 조건이 맞는 공고에 전부
 *  응모하는 것이 늘 정답이라 이 화면이 선택이 아니라 버튼 누르기가 된다(`POST_AP`와
...
export const LISTING_TIERS = [
  {
    ...
    /** 경쟁률(응모자 수 느낌). 확률의 바닥을 정한다 — 아래 `BID_BASE`와 함께 읽는다. */
    rivals: 3,
  },
  {
    id: 'medium',
...
/** ── 당첨 확률 ─────────────────────────────────────────────
 *
 * **회사평판 + 능력치**가 정한다(사용자 원문). 화면은 응모 전에 이 값을 적는다 —
 ...
 *
 * ⚠️ **0도 1도 되지 않는다.** 0이면 조건을 맞춰 응모했는데 이길 길이 없어 응모가 뜻을
...
/** 공고의 마감(주). ⚠️ **당첨된 뒤부터 센다** — 응모하고 기다린 주는 기한을 먹지 않는다
 *  (`acceptJob`이 `dueWeeks`를 받는 순간부터 굳는다). */
