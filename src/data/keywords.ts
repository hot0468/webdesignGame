 ...
 *
 * 고리는 이렇다:
 *   사이트 수주 → **클라이언트 미팅**(행동력 `MEETING_MINS`)에서 원하는 키워드 몇 개를 알아냄
...
/** 클라이언트 미팅 1회 비용(분). ⚠️ 제작 공정이 아니므로 짧다 — 미팅이 시안만큼
 *  비싸면 아무도 하지 않고, 그러면 이 기능이 화면에만 남는다.
 *  ⚠️ 한 시간은 **자투리에 안 들어가는 크기**다(짧은 손짓 30분과 갈린다) — 미팅은
 *     오후를 통째로 비우진 않지만 그날 계획에는 들어와야 한다. */
export const MEETING_MINS = 60
